  ...
  useEffect(() => {
    if (game) {
      ...
      setForm({
        ...
        notes: game.notes || '',
        jangandibeli: game.jangandibeli === true
      });
  ...
  async function handleSave(closeAfter) {
    ...
    try {
      const payload = {
        ...
        jangandibeli: form.jangandibeli
      };
    ...
    <div className="modal-overlay" onMouseDown={handleOverlayMouseDown}>
      <div className="modal-panel" style={{ maxWidth: 760 }} onMouseDown={(e) => e.stopPropagation()}>
        ...
        <div className="modal-body">
          ...
          {/* Jangan Dibeli */}
          <div className="form-group">
            <label className="jangan-dibeli-label" style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer', userSelect: 'none' }}>
              <input
                type="checkbox"
                checked={form.jangandibeli}
                onChange={(e) => update('jangandibeli', e.target.checked)}
                style={{ width: 16, height: 16, accentColor: 'var(--color-danger, #e53e3e)', cursor: 'pointer' }}
              />
              <span style={{ fontWeight: 600, color: form.jangandibeli ? 'var(--color-danger, #e53e3e)' : 'var(--color-text-muted)' }}>
                Jangan Dibeli
              </span>
              {form.jangandibeli && (
                <span style={{ fontSize: 12, color: 'var(--color-danger, #e53e3e)', fontWeight: 400 }}>
                  — kartu akan ditandai dengan tanda ✕
                </span>
              )}
            </label>
          </div>
          ...
          {(() => {
            ...
            const offline = Number(form.sellPriceOffline) || 0;
            const shopee = Number(form.sellPriceShopee) || 0;
            const warnings = [];
            if (buy > 0 && offline > 0 && buy > offline) {
              warnings.push('⚠️ Harga Beli lebih tinggi dari Jual Offline');
            }
            if (offline > 0 && shopee > 0 && offline > shopee) {
              warnings.push('⚠️ Jual Offline lebih tinggi dari Setting Shopee');
            }
