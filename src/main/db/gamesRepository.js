...
function rowToGame(row) {
  ...
  return {
    ...
    jangandibeli: row.jangan_dibeli === 1,
  ...
  db.prepare(
    `INSERT INTO games
      (id, name, platform, platform_custom, region, condition,
       ...
       cover_image_id, is_deleted, jangan_dibeli, created_at, updated_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NULL, 0, ?, ?, ?)`
  ).run(
    id,
    input.name,
  ...
  const tx = db.transaction(() => {
    ...
    db.prepare(
      `UPDATE games SET
        ...
        buy_price = ?, sell_price_offline = ?, sell_price_shopee = ?, notes = ?,
        cover_image_id = ?, jangan_dibeli = ?, updated_at = ?
       WHERE id = ?`
    ).run(
      ...
      input.jangandibeli !== undefined ? (input.jangandibeli ? 1 : 0) : existing.jangan_dibeli,
      ts,
      id
    );
  });
...
/**
 * Hitung ulang sell_price_shopee dari sell_price_offline menggunakan formula:
 *   raw = sell_price_offline / (1 - adminFeePercent/100)
 *   dibulatkan ke kelipatan 5000 terdekat
 * ids: array ID yang akan diupdate. Jika kosong = semua kartu aktif.
 */
function batchRecalcShopee({ ids, adminFeePercent }) {
  const db = getDb();
  const ts = nowIso();
  ...
  function calcShopeePrice(jualOffline) {
  ...
  const tx = db.transaction(() => {
    ...
    if (ids && ids.length > 0) {
      ...
      rows = db.prepare(
        `SELECT id, sell_price_offline, sell_price_shopee FROM games WHERE id IN (${placeholders}) AND is_deleted = 0`
      ).all(...ids);
    } else {
      rows = db.prepare(
        `SELECT id, sell_price_offline, sell_price_shopee FROM games WHERE is_deleted = 0`
      ).all();
    }

    const updateStmt = db.prepare(`UPDATE games SET sell_price_shopee = ?, updated_at = ? WHERE id = ?`);
    for (const row of rows) {
      const newShopee = calcShopeePrice(row.sell_price_offline);
      recordPriceHistory(db, row.id, 'sell_price_shopee', row.sell_price_shopee, newShopee, ts);
      updateStmt.run(newShopee, ts, row.id);
    }
    return rows.length;
  });
